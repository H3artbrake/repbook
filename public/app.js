...
async function start(id){
  ...
  state.active={id:uid(),templateName:t.name,date:today(),notes:'',exercises:t.exerciseIds.map(id=>{const e=state.exercises.find(e=>e.id===id);return {exerciseId:id,name:e.name,sets:[makeSet(e)]};})};
...
function exercisePanel(e,ei,session){
  ...
  return `<article class="panel exercise-panel"><div class="exercise-heading"><div><h3><button class="exercise-link" data-action="open-progress" data-id="${e.exerciseId}" aria-label="View progress for ${esc(e.name)}">${esc(e.name)} <span aria-hidden="true">↗</span></button></h3><small>${esc(def.group)}</small></div><button class="remove-set" data-action="remove-exercise" data-e="${ei}" aria-label="Remove ${esc(e.name)}">×</button></div><div class="last">${prev?`<strong>Last time</strong> · ${dateLabel(prev.date)}<br>${completedSets(prev).map(setLabel).map(esc).join(' &nbsp; / &nbsp; ')}`:'No previous sets yet. This is your starting point.'}</div><div class="sets"><div class="set-row header" aria-hidden="true"><span>Set</span><span>kg</span><span>Reps</span><span></span></div>${e.sets.map((set,si)=>`<div class="set-row ${set.done?'logged':''}"><span class="set-index">${si+1}</span><input aria-label="${esc(e.name)} set ${si+1} kilograms" data-field="weight" data-e="${ei}" data-s="${si}" type="number" inputmode="decimal" min="-2000" max="2000" step="0.25" placeholder="BW" value="${set.weight===null?'':set.mode==='assisted'?-set.weight:set.weight}"><input aria-label="${esc(e.name)} set ${si+1} reps" data-field="reps" data-e="${ei}" data-s="${si}" type="number" inputmode="numeric" min="1" max="1000" step="1" placeholder="—" value="${set.reps??''}"><button class="remove-set" data-action="remove-set" data-e="${ei}" data-s="${si}" aria-label="Delete set ${si+1}">×</button></div>`).join('')}</div><div class="exercise-footer"><button class="small" data-action="add-set" data-e="${ei}">＋ Add set</button>${prev?`<button class="small ghost" data-action="copy-last" data-e="${ei}">Use last session</button>`:''}</div></article>`;
}
